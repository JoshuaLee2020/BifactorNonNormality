...
################################################################################
sim_bf <- function(Factor,I,N,iter){
  ...
  SkewLevel.name <- c("Normal","Moderate","Severe")
   ...
   for (Fg in 1:G){
    for (Fs in 1:S){
      ...
      data.3.folder <- paste0(data.2.folder,Factor,"-",I,"-",N,"-",SkewLevel.name[Fg],"-",SkewLevel.name[Fs],"/")
      ...
      for (iter in 1:iter){ 
        ...
        if (SkewLevel[Fs]==0){kurt.Fs = 0} else {kurt.Fs = 7}      
        ...
        theta.sim  <- covsim::rPLSIM(N, sigma.target, skewness=c(SkewLevel[Fg], rep(SkewLevel[Fs], K-1)),
                                      excesskurtosis=c(kurt.Fg,rep(kurt.Fs,K-1)))
        theta  <- theta.sim[[1]][[1]]
        #theta[,1]
        ...
        parameters.name <- paste0(data.3.folder,Factor,"-",I,"-",N,"-",SkewLevel.name[Fg],"-",SkewLevel.name[Fs],"-",iter,"-realparameters.csv")      
        write.table(parameters,file = parameters.name,sep=",",row.names=FALSE, col.names=FALSE)
          
        ### write theta of real (population)  file    
        theta.name <- paste0(data.3.folder,Factor,"-",I,"-",N,"-",SkewLevel.name[Fg],"-",SkewLevel.name[Fs],"-",iter,"-realtheta.csv")
        write.table(theta,file = theta.name,sep=",",row.names=FALSE, col.names=FALSE)
          
        ### write data (response scores) file
        data.name <- paste0(Factor,"-",I,"-",N,"-",SkewLevel.name[Fg],"-",SkewLevel.name[Fs],"-",iter,"-data.csv")
        ...
        syn.name <- paste0(data.3.folder,"Syn",Factor,"-",I,"-",N,"-",SkewLevel.name[Fg],"-",SkewLevel.name[Fs],"-",iter,".flexmirt")
